...
import machine
import network
import utime  # Import utime to get timestamps
...
def log_to_csv(message: str, rssi: int) -> None:
    """
    Logs the given message and RSSI value to a CSV file with a timestamp.
    """
    try:
        # Get the current timestamp as a tuple (year, month, day, hour, minute, second, ...)
        ...
        with open("log.csv", "a") as f:
            f.write("{},{},{}\n".format(timestamp_str, message, rssi))
...
def check_name(result: tuple) -> None:
    ...
    for mac, info in esp.peers_table.items():
        ...
        if rssi > -88:
            ...
            # Save a log to CSV when RSSI > -88
            log_to_csv(msg, rssi)
